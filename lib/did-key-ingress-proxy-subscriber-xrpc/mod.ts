...
import type {
  ...
  CallerOptions,
  CallerHandle,
} from "@publicdomainrelay/did-key-ingress-proxy-subscriber-abc";
import type { RelayRequest } from "@publicdomainrelay/did-key-ingress-proxy-common";
...
export async function createSubscriber(
  ...
  const hostname = hostnameOnly(opts.ingressProxyHost);
  const ingressRef = `did:web:${subdomain}.${hostname}`;
  ...
  function openOnce(): Promise<void> {
    return (async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        ws.onopen = () => {
          log.info("ws_open", { component: label, host: opts.ingressProxyHost });
        };
        ws.onmessage = (evt) => {
...
export function tunnelOverRelay(opts: TunnelClientOptions): Promise<void> {
  ...
  const host = `${opts.subscriberSubdomain}.${opts.ingressProxyHost}`;
...
export function createCaller(opts: CallerOptions): CallerHandle {
  ...
  const hostname = hostnameOnly(opts.ingressProxyHost);
  const url = new URL(
    `wss://${subdomain}.${opts.ingressProxyHost}/xrpc/${nsid}`,
  );
